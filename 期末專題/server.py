...
app.config['KEEP_FOLDER'] = 'keep'
os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)
os.makedirs(app.config['TRASH_FOLDER'], exist_ok=True)
os.makedirs(app.config['KEEP_FOLDER'], exist_ok=True)
...
def index(subpath=''):
    folder_path = app.config['KEEP_FOLDER'] if subpath == 'keep' else os.path.join(app.config['UPLOAD_FOLDER'], subpath)
...
@app.route('/download_keep/<path:filename>')
def download_keep_file(filename):
    return send_from_directory(app.config['KEEP_FOLDER'], filename)
...
def handle_file_list(data):
    ...
    if subpath == 'trash':
        folder_path = os.path.join(app.config['TRASH_FOLDER'])
    elif subpath == 'keep':
        folder_path = os.path.join(app.config['KEEP_FOLDER'])
    else:
...
def handle_open_file(data):
    if data['subpath'] == 'keep':
        filepath = os.path.join(app.config['KEEP_FOLDER'], data['filename'])
    else:
...
def handle_save_file(data):
    ...
    if os.path.exists(os.path.join(app.config['KEEP_FOLDER'], filename)):
        with open(os.path.join(app.config['KEEP_FOLDER'], filename), 'w') as file:
            file.write(data['content'])
...
def handle_delete_file(data):
    ...
    if os.path.exists(os.path.join(app.config['KEEP_FOLDER'], filename)):
        os.remove(os.path.join(app.config['KEEP_FOLDER'], filename))
...
@socketio.on('keep_file')
def handle_keep_file(data):
    filename = data['filename']
    subpath = data['subpath']
    src_path = os.path.join(app.config['UPLOAD_FOLDER'], subpath, filename)
    dest_path = os.path.join(app.config['KEEP_FOLDER'], filename)
    shutil.copy(src_path, dest_path)
    emit('file_kept', {'filename': filename})

@socketio.on('unkeep_file')
def handle_unkeep_file(data):
    filename = data['filename']
    src_path = os.path.join(app.config['KEEP_FOLDER'], filename)
    os.remove(src_path)
    emit('file_unkept', {'filename': filename})
